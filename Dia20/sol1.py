...
for i in range(len(l)):
    ...
    if node.num%len(l) == 0:
        continue
    # node_to_move = find_node(node.num,node)
    if node.num < 0:
        node_to_move = find_node(-1 * (abs(node.num)%len(l)),node)
    else:
        node_to_move = find_node(node.num%len(l),node)
...
print(find_node(1001,next_node).num+find_node(2001,next_node).num+find_node(3001,next_node).num )
